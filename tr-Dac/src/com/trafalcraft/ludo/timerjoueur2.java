	...
	public static void startCountdown2(final int CAB2) {
		task2 = Bukkit.getScheduler().scheduleSyncDelayedTask(mainClasse.plugin, new Runnable() {
			...
			public void run() {
				...
				if (mainClasse.a5 == 1) {
					mainClasse.plugin.player2.setVie(mainClasse.plugin.player2.getVie() - 1);
					...
					Player a8 = mainClasse.plugin.player2.getJoueur();
					...
					if (mainClasse.cf1 == 1 || mainClasse.cf2 == 1 || mainClasse.cf3 == 1 || mainClasse.cf4 == 1
						...
						if (mainClasse.plugin.player2.getVie() >= 1) {
							...
							Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §b"
									+ mainClasse.plugin.player2.getNom() + " n'a pas réussit a confirmer");
							if (mainClasse.cf1 == 1) {
					...
					if (mainClasse.plugin.player2.getVie() == 0) {
						if (mainClasse.nbrJoueur == 2) {
							...
							if (a15 > mainClasse.plugin.player2.getNumero()) {
								Bukkit.getServer()
										.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom()
												+ " a raté(e) son saut il lui reste "
												+ mainClasse.plugin.player2.getVie() + " vie");
								...
								if (mainClasse.cf1 == 1 || mainClasse.cf2 == 1 || mainClasse.cf3 == 1
									...
									Bukkit.getServer()
											.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom()
													...
													+ mainClasse.plugin.player2.getVie() + " vie");
									Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §b"
											+ mainClasse.plugin.player2.getNom() + " est eliminé(e)");
							...
							Bukkit.getServer()
									.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom()
											+ " a raté(e) son saut il lui reste " + mainClasse.plugin.player2.getVie()
											+ " vie");
							Bukkit.getServer().broadcastMessage(
									"§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom() + " est eliminé(e)");
							mainClasse.nbrJoueur--;
						}
					} else {
						Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom()
								+ " a raté(e) son saut il lui reste " + mainClasse.plugin.player2.getVie() + " vie");
					}
					...
					mainClasse.plugin.player2.getJoueur().setHealth(20);
					mainClasse.plugin.player2.setEnJeu(0);
				...
				if (mainClasse.a6 == 1) {
					...
					Bukkit.getWorld(a8.getWorld().getName()).spawnFallingBlock(a8.getLocation(), Material.WOOL,
							(byte) 14);
					Bukkit.getWorld(a8.getWorld().getName()).spawnFallingBlock(blockLocation, Material.WOOL, (byte) 14);

					a8.teleport(new Location(world, x, y, z));

					Bukkit.getServer()
							.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom()
									+ " a réussit son saut, il lui rest toujours " + mainClasse.plugin.player2.getVie()
									+ "vie(s)");
					...
					if (mainClasse.cf1 == 1 || mainClasse.cf2 == 1 || mainClasse.cf3 == 1 || mainClasse.cf4 == 1
						...
						Bukkit.getServer().broadcastMessage(
								"§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom() + " a confirmer");
				...
				if (mainClasse.a7 == 1) {
					...
					Bukkit.getServer()
							.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player2.getNom()
									...
									+ mainClasse.plugin.player2.getVie() + " vie(s)");
				...
				if (time2 == 10) {
					Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §bil reste 10 seconds à "
							+ mainClasse.plugin.player2.getNom() + " pour sauter");
				}
