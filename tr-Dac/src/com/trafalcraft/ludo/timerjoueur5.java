	...
	public static void startCountdown2(final int CAB2) {
		task2 = Bukkit.getScheduler().scheduleSyncDelayedTask(mainClasse.plugin, new Runnable() {
			...
			public void run() {
				...
				if (mainClasse.a5 == 1) {
					mainClasse.plugin.player5.setVie(mainClasse.plugin.player5.getVie() - 1);
					...
					Player a8 = mainClasse.plugin.player5.getJoueur();
					...
					if (mainClasse.plugin.player5.getVie() == 0) {
						if (mainClasse.nbrJoueur == 2) {
							...
							if (a15 > mainClasse.plugin.player5.getNumero()) {
								Bukkit.getServer()
										.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom()
												+ " a raté(e) son saut il lui reste "
												+ mainClasse.plugin.player5.getVie() + " vie");
								...
								if (mainClasse.cf1 == 1 || mainClasse.cf2 == 1 || mainClasse.cf3 == 1
									...
									Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §b"
											+ mainClasse.plugin.player5.getNom() + " n'a pas réussit a confirmer");
									if (mainClasse.cf1 == 1) {
										mainClasse.plugin.player1.setVie(1);
										mainClasse.plugin.player5.setVie(mainClasse.plugin.player5.getVie() + 1);
									}
									...
									Bukkit.getServer()
											.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom()
													...
													+ mainClasse.plugin.player5.getVie() + " vie");
									Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §b"
											+ mainClasse.plugin.player5.getNom() + " est eliminé(e)");
							...
							Bukkit.getServer()
									.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom()
											+ " a raté(e) son saut il lui reste " + mainClasse.plugin.player5.getVie()
											+ " vie");
							Bukkit.getServer().broadcastMessage(
									"§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom() + " est eliminé(e)");
							mainClasse.nbrJoueur--;
						}
					} else {
						Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom()
								+ " a raté(e) son saut il lui reste " + mainClasse.plugin.player5.getVie() + " vie");
					}
					mainClasse.a5 = 0;
					mainClasse.a6 = 0;
					mainClasse.a7 = 0;

					mainClasse.plugin.player5.getJoueur().setHealth(20);
					mainClasse.plugin.player5.setEnJeu(0);
				...
				if (mainClasse.a6 == 1) {
					...
					Bukkit.getWorld(a8.getWorld().getName()).spawnFallingBlock(a8.getLocation(), Material.WOOL,
							(byte) 5);
					Bukkit.getWorld(a8.getWorld().getName()).spawnFallingBlock(blockLocation, Material.WOOL, (byte) 5);

					a8.teleport(new Location(world, x, y, z));

					Bukkit.getServer()
							.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom()
									+ " a réussit son saut, il lui rest toujours " + mainClasse.plugin.player5.getVie()
									+ "vie(s)");
					...
					if (mainClasse.cf1 == 1 || mainClasse.cf2 == 1 || mainClasse.cf3 == 1 || mainClasse.cf4 == 1
						...
						Bukkit.getServer().broadcastMessage(
								"§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom() + " a confirmer");
				...
				if (mainClasse.a7 == 1) {
					...
					Bukkit.getWorld(a8.getWorld().getName()).spawnFallingBlock(a8.getLocation(), Material.WOOL,
							(byte) 5);
					Bukkit.getWorld(a8.getWorld().getName()).spawnFallingBlock(blockLocation, Material.GLASS, (byte) 5);
					...
					Bukkit.getServer()
							.broadcastMessage("§3§ldé à coudre> §b" + mainClasse.plugin.player5.getNom()
									...
									+ mainClasse.plugin.player5.getVie() + " vie(s)");
				...
				if (time2 == 10) {
					Bukkit.getServer().broadcastMessage("§3§ldé à coudre> §bil reste 10 seconds à "
							+ mainClasse.plugin.player5.getNom() + " pour sauter");
				}
