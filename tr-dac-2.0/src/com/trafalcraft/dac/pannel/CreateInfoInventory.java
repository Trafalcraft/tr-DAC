...
public class CreateInfoInventory {
	...
	public static void loadInventoryInfo(Player p, String arene){
		...
		Inventory inv = Bukkit.createInventory(p, 36, arene + ";infoInventory;dac");
		...
		if(FileControler.getArena(arene).contains("mode")){
			
			ItemStack item = new ItemStack(Material.PAPER);		
			ItemMeta i = item.getItemMeta();
			i.setDisplayName("mode");
			
			List<String> lore = new ArrayList<String>();
			lore.add(FileControler.getArena(arene).getString("mode") + "");
			i.setLore(lore);
